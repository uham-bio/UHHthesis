...
[flextable](https://ardata-fr.github.io/flextable-book/) creates tables that work well in both Word and PDF output. Table \@ref(tab:flextable1) shows an example:

```{r flextable1}
df <- mtcars[1:5, 1:6]
flextable(df) |>
  set_caption(caption = "Overview of the first lines and columns in the mtcars dataset.") |>
  autofit() |>
  theme_booktabs() |>
  bold(part = "header")
```

> **Tip:** Another excellent table package is [gt](https://gt.rstudio.com/), which provides a tidyverse-friendly grammar of tables.


## Figures

Figures can directly be produced with R and displayed here. The [ggplot2](https://ggplot2.tidyverse.org/) package provides a powerful and flexible system for creating publication-quality graphics. Similar to external images, figure captions and labels are placed inside the chunk options for cross-referencing (see Fig. \@ref(fig:scatter-fig)).

```{r scatter-fig, fig.cap = "Relationship between horsepower and fuel economy."}
